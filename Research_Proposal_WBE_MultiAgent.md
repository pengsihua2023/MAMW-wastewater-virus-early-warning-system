...
Build, document, and containerize a nine-agent AutoGen v0.4 system that executes the complete wastewater mNGS workflow, from raw FASTQ ingestion to alert generation and Word/PDF report production. Deliverable: open-source platform deposited on GitHub with full documentation and test datasets.
...
Assess system runtime on CDC-compatible HPC (SLURM, Apptainer), cost per sample, staff time saved versus manual pipeline operation, and epidemiologist acceptability of AI-generated alert reports. Develop and test a human-in-the-loop approval workflow for ORANGE and RED alerts.
...
The platform adopts a three-tier federated design:

```
Tier 1 — Edge (laboratory sites)
  Raw mNGS data generation → local host depletion (HRRT) → privacy-scrubbed FASTQ

Tier 2 — Regional HPC nodes
  Agent execution (Kraken2, MEGAHIT, Freyja, iVar) via SLURM/Apptainer
  Site-level trend databases (SQLite → PostgreSQL)

Tier 3 — CDC Central (1CDP cloud)
  Orchestrator Agent → Alert synthesis → Dashboard → Report distribution
```

Tier boundaries enforce data minimization: raw reads never leave Tier 1. Tier 2 transmits classified counts and variant tables. Tier 3 receives normalized metrics and agent-generated narratives only.

### 4.2 Agent Roles and Implementation

The platform comprises nine agents, each with a defined role, tool set, and structured output contract:
...
All agents communicate through AutoGen's structured message protocol. The **Executor** (UserProxyAgent) is the sole node permitted to invoke shell commands, enforcing a clean separation between reasoning and execution.

**Alert scoring** combines three independent evidence streams: (1) absolute abundance of a flagged pathogen exceeding site-specific 90th-percentile baseline, (2) epidemiological anomaly (Z-score > 3 or week-over-week growth > 50%), and (3) presence of a lineage not detected in the prior four weeks. At least two independent streams must trigger before an ORANGE or RED alert is issued, preventing single-datapoint escalation.
